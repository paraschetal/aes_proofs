...
(define xor_bytes (lambdas (b a)
  (match b
     ((Bits8 b7 b6 b5 b4 b3 b2 b1 b0)
       (match a
          ((Bits8 a7 a6 a5 a4 a3 a2 a1 a0) `(Bits8 ,(@ xor_bits b7 a7)
            ,(@ xor_bits b6 a6) ,(@ xor_bits b5 a5) ,(@ xor_bits b4 a4)
            ,(@ xor_bits b3 a3) ,(@ xor_bits b2 a2) ,(@ xor_bits b1 a1)
            ,(@ xor_bits b0 a0))))))))
...
(define inv_shift_rows (lambda (state)
  (match state
     ((Bytes16 r0c0 r0c1 r0c2 r0c3 r1c0 r1c1 r1c2 r1c3 r2c0 r2c1 r2c2 r2c3 r3c0 r3c1
       r3c2 r3c3) `(Bytes16 ,r0c0 ,r0c1 ,r0c2 ,r0c3 ,r1c3 ,r1c0 ,r1c1 ,r1c2 ,r2c2 ,r2c3
       ,r2c0 ,r2c1 ,r3c1 ,r3c2 ,r3c3 ,r3c0)))))

(define inv_sub_bytes (lambda (state)
  (match state
     ((Bytes16 r0c0 r0c1 r0c2 r0c3 r1c0 r1c1 r1c2 r1c3 r2c0 r2c1 r2c2 r2c3 r3c0 r3c1
       r3c2 r3c3) `(Bytes16 ,(inv_s_box r0c0) ,(inv_s_box r0c1) ,(inv_s_box r0c2)
       ,(inv_s_box r0c3) ,(inv_s_box r1c0) ,(inv_s_box r1c1) ,(inv_s_box r1c2)
       ,(inv_s_box r1c3) ,(inv_s_box r2c0) ,(inv_s_box r2c1) ,(inv_s_box r2c2)
       ,(inv_s_box r2c3) ,(inv_s_box r3c0) ,(inv_s_box r3c1) ,(inv_s_box r3c2)
       ,(inv_s_box r3c3))))))
  ...
  (match b
     ((Bits8 b7 b6 b5 b4 b3 b2 b1 b0)
       (match b7
          ...
          ((S1)
            (@ xor_bytes `(Bits8 ,b6 ,b5 ,b4 ,b3 ,b2 ,b1 ,b0 ,`(S0)) `(Bits8 ,`(S0)
              ,`(S0) ,`(S0) ,`(S1) ,`(S1) ,`(S0) ,`(S1) ,`(S1)))))))))

(define mul02 xtime)

(define mul09 (lambda (b) (@ xor_bytes b (mul02 (mul02 (mul02 b))))))

(define mul0b (lambda (b)
  (@ xor_bytes b (@ xor_bytes (mul02 b) (mul02 (mul02 (mul02 b)))))))

(define mul0d (lambda (b)
  (@ xor_bytes b (@ xor_bytes (mul02 (mul02 b)) (mul02 (mul02 (mul02 b)))))))

(define mul0e (lambda (b)
  (@ xor_bytes (mul02 b) (@ xor_bytes (mul02 (mul02 b)) (mul02 (mul02 (mul02 b)))))))

(define inv_mix_column_transform (lambda (column)
  (match column
     ((Bytes4 a~ b~ c~ d~)
       (let ((a~~
         (@ xor_bytes (@ xor_bytes (@ xor_bytes (mul0e a~) (mul0b b~)) (mul0d c~))
           (mul09 d~))))
         (let ((b~~
           (@ xor_bytes (@ xor_bytes (@ xor_bytes (mul09 a~) (mul0e b~)) (mul0b c~))
             (mul0d d~))))
           (let ((c~~
             (@ xor_bytes (@ xor_bytes (@ xor_bytes (mul0d a~) (mul09 b~)) (mul0e c~))
               (mul0b d~))))
             (let ((d~~
               (@ xor_bytes (@ xor_bytes (@ xor_bytes (mul0b a~) (mul0d b~)) (mul09 c~))
                 (mul0e d~)))) `(Bytes4 ,a~~ ,b~~ ,c~~ ,d~~)))))))))
          ...
          ((Bytes4 c01 c11 c21 c31)
            (match c2
               ((Bytes4 c02 c12 c22 c32)
                 (match c3
                    ((Bytes4 c03 c13 c23 c33) `(Bytes16 ,c00 ,c01 ,c02 ,c03 ,c10 ,c11
                      ,c12 ,c13 ,c20 ,c21 ,c22 ,c23 ,c30 ,c31 ,c32 ,c33)))))))))))

(define inv_mix_columns (lambda (state)
  (match state
     ((Bytes16 s00 s01 s02 s03 s10 s11 s12 s13 s20 s21 s22 s23 s30 s31 s32 s33)
       (let ((ncol0 (inv_mix_column_transform `(Bytes4 ,s00 ,s10 ,s20 ,s30))))
...
(define rcon (lambda (i) `(Bytes4 ,(rc i) ,`(Bits8 ,`(S0) ,`(S0) ,`(S0) ,`(S0) ,`(S0)
  ,`(S0) ,`(S0) ,`(S0)) ,`(Bits8 ,`(S0) ,`(S0) ,`(S0) ,`(S0) ,`(S0) ,`(S0) ,`(S0)
...
(define xor_words (lambdas (w1 w2)
  (match w1
     ((Bytes4 b0 b1 b2 b3)
       (match w2
          ((Bytes4 a0 a1 a2 a3) `(Bytes4 ,(@ xor_bytes b0 a0) ,(@ xor_bytes b1 a1)
            ,(@ xor_bytes b2 a2) ,(@ xor_bytes b3 a3))))))))
...
(define matrix_to_keyt (lambda (k)
  (match k
     ((Bytes16 b11 b12 b13 b14 b21 b22 b23 b24 b31 b32 b33 b34 b41 b42 b43 b44)
       `(Keywords ,`(Bytes4 ,b11 ,b12 ,b13 ,b14) ,`(Bytes4 ,b21 ,b22 ,b23 ,b24)
               ...
               ((Bytes4 b21 b22 b23 b24)
                 (match w3
                    ((Bytes4 b31 b32 b33 b34)
                      (match w4
                         ((Bytes4 b41 b42 b43 b44) `(Bytes16 ,b11 ,b12 ,b13 ,b14 ,b21
                           ,b22 ,b23 ,b24 ,b31 ,b32 ,b33 ,b34 ,b41 ,b42 ,b43 ,b44)))))))))))))
...
(define rk (lambdas (i k)
  (match k
     ((Keywords w1 w2 w3 w4) `(Keywords ,(@ xor_words (@ first_w_in_rk i w4) w1)
       ...
       ,(@ xor_words
          (@ xor_words (@ xor_words (@ xor_words (@ first_w_in_rk i w4) w1) w2) w3) w4))))))
...
(define add_round_key (lambdas (k state)
  (match k
     ((Bytes16 k00 k01 k02 k03 k10 k11 k12 k13 k20 k21 k22 k23 k30 k31 k32 k33)
       (match state
          ((Bytes16 s00 s01 s02 s03 s10 s11 s12 s13 s20 s21 s22 s23 s30 s31 s32 s33)
            `(Bytes16 ,(@ xor_bytes k00 s00) ,(@ xor_bytes k01 s01)
            ,(@ xor_bytes k02 s02) ,(@ xor_bytes k03 s03) ,(@ xor_bytes k10 s10)
            ,(@ xor_bytes k11 s11) ,(@ xor_bytes k12 s12) ,(@ xor_bytes k13 s13)
            ,(@ xor_bytes k20 s20) ,(@ xor_bytes k21 s21) ,(@ xor_bytes k22 s22)
            ,(@ xor_bytes k23 s23) ,(@ xor_bytes k30 s30) ,(@ xor_bytes k31 s31)
        ...
        (@ dec_round7 k
          (@ dec_round6 k
            (@ dec_round5 k
              (@ dec_round4 k
                (@ dec_round3 k
                  (@ dec_round2 k (@ dec_round1 k (@ add_round_key (key10 k) c)))))))))))))

(define dec_aes_cbc (lambdas (key iv ciphertext)
  (match ciphertext
